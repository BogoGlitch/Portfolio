...
### Completed
- **Observability + resilience** — Application Insights telemetry (auto-instruments requests, SQL dependencies, exceptions; Serilog output captured via ILogger pipeline — no dedicated sink); EF Core `EnableRetryOnFailure` (5 retries, 30s max backoff) for Azure SQL transient errors; connection string in Key Vault as `ApplicationInsights--ConnectionString`; test suite updated to 132 passing (Discipline field gaps fixed, retry strategy test added)
